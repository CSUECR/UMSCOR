...
namespace HOHO18.Common
{
    public static class ChineseDate
    {
        ...
        /// <param name="stringFormat">[year],[month][year]</param>
        /// <returns></returns>
        public static string ToChineseDateTime(this DateTime datetime,string stringFormat="")
        {
            //农历的年月日
            ...
            {
                return stringFormat.Replace("[year]", GetLunisolarYear(year)).Replace("[month]", (isleap ? "闰" : string.Empty)+GetLunisolarMonth(month))
                    .Replace("[day]", GetLunisolarDay(day));
            }
        }
    }
}
